...
- **Stage 3**: **`WordpressInstanceAutoIP`** output and **`WordpressURL`** wording aligned with Stages 1–2; optional **`WordpressDbDumpS3Bucket`** / **`WordpressDbDumpS3Key`** + **`HasDbDumpMigration`** for same-stack **`update-stack` from stage 2** (S3-hosted **`mysqldump`** → RDS import in UserData); public subnets aligned with Stages 1–2 (**`AssignIpv6AddressOnCreation`**, no IPv6 workaround Lambda); default AMI path **kernel 6.18**
...
### Removed
- **Stages 1–2 only**: IPv6 subnet helper Lambda + custom resources on public subnets (replaced by **`AssignIpv6AddressOnCreation`** and **`Ipv6CidrBlock`** on each public subnet).
- **Stage 3 (public subnets)**: same IPv6 workaround Lambda + custom resources as older stage 3 revisions (public subnets now match Stages 1–2; **Stages 4–5** still use the workaround on public subnets)
